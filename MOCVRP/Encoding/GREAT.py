...
class GREATEncoder(nn.Module):
    ...
    def forward(self, dists, demands):
        batch_size, n_nodes, _ = dists.shape

        # Augment edge information with demand
        demands_expanded = demands.unsqueeze(1).expand(batch_size, n_nodes, n_nodes).unsqueeze(3)
        problems = torch.cat((dists.unsqueeze(3), demands_expanded), dim=3)
        # (B, N, N, 2)
