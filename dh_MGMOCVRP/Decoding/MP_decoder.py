...
class MPDecoder(torch.nn.Module):
    ...
    def forward(self, last_node, group_ninf_mask, current_load):
        ...
        current_load_exp = current_load.unsqueeze(2).clone()
        current_load_emb = self.load_embedder(current_load_exp)
        final_q = final_q + current_load_emb
