...
from dh_MGMOCVRP.MGMOCVRPTester import CVRPTester
...
def main(n_sols = 101):
    ...
    if tester_params['data_load_path'] is not None:
        shared_problem, edge_to_node, demands = load_problems(tester_params['data_load_path'])
        edge_attr = shared_problem.to(tester.device)
        edge_to_node = edge_to_node.to(tester.device)
        demands = demands.to(tester.device)
    else:
        shared_problem, edge_to_node, demands = get_random_problems(env_params['distribution'], tester_params['test_episodes'], env_params['problem_size'], env_params['emax'])
