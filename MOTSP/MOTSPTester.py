...
from MOTSPProblemDef import augment_data
...
class TSPTester:
    ...
    def run(self, shared_problem, prefs, print_results = True):
        ...
        while episode < test_num_episode:
            ...
            sols = self._test_one_batch(shared_problem, batch_size, episode)
    ...
    def _test_one_batch(self, shared_problem, batch_size, episode):
        ...
        self.env.problems = shared_problem[episode: episode + batch_size, :, :, :]

        if aug_factor > 1:
            self.env.batch_size = self.env.batch_size * aug_factor
            self.env.problems, aug_factors = augment_data(self.env.problems, aug_factor)
