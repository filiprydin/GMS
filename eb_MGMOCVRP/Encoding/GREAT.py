...
class GREATEncoder(nn.Module):
    ...
    def forward(self, problems, edges_to_nodes, num_nodes, emax=1):
        # problems: (B, E, N_obj)
        # edges_to_nodes: (B, E, 2)

        edge_attr, edge_index = format_encoder_input(problems, edges_to_nodes, num_nodes)
        # (B * E, N_obj) and (2, B * E)
...
def format_encoder_input(dists, indices, n_nodes):
    ...
    edge_attr = dists.reshape(-1, N)  # (B * E, N)
