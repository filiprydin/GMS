...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, P, pref=None, encode=True):
        ...
        demand = reset_state.demands # (B, N)
        batch_size, self.n_edges, _ = edge_attr.shape

        if encode:
            # Add demand as edge features
            demand_features = torch.gather(demand, 1, edge_to_node[:, :, 1]).unsqueeze(-1)
            edge_attr_aug = torch.cat((edge_attr, demand_features), dim = 2)
    ...
    def forward(self, state):
        ...
        probs, indices, embeddings = self.decoder(self.last_edge_embedding, state.current_node, ninf_mask, state.load)
        ...
        # If finished, force prob to 1.0 since rest of the actions are just nonsense
        if prob is not None:
            finished = state.finished
            prob = prob.masked_fill(finished, 1.0)
