...
from eb_MGMOCVRP.MGMOCVRPTester import CVRPTester
...
def load_problems(path):
    ...
    return edge_dists, edge_to_node, demands
...
def main(n_sols = 101):
    ...
    if tester_params['data_load_path'] is not None:
        edge_attr, edge_to_node, demands = load_problems(tester_params['data_load_path'])
        edge_attr = edge_attr.to(tester.device)
        edge_to_node = edge_to_node.to(tester.device).to(dtype=torch.int64)
        demands = demands.to(tester.device)
    else:
        edge_attr, edge_to_node, demands = get_random_problems(env_params['distribution'], tester_params['test_episodes'], env_params['problem_size'], env_params['emax'])
    ...
    sols = tester.run(edge_attr, edge_to_node, demands, prefs, print_results=False)
