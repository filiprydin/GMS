...
from eb_MGMOCVRP.MGMOCVRPProblemDef import augment_data
...
class CVRPTester:
    def __init__(self,
        ...
        self.ref = tester_params['reference']
    ...
    def run(self, edge_attr, edge_to_node, demands, prefs, print_results=True):
        ...
        while episode < test_num_episode:
            ...
            sols = self._test_one_batch(edge_attr, edge_to_node, demands, batch_size, episode)
    ...
    def _test_one_batch(self, edge_attr, edge_to_node, demands, batch_size, episode):
        ...
        self.env.demands = demands[episode: episode + batch_size, :]
        
        if aug_factor > 1:
            self.env.batch_size = self.env.batch_size * aug_factor
            self.env.edge_attr, self.env.edge_to_node, self.env.demands, aug_factors = augment_data(self.env.edge_attr, self.env.edge_to_node,
                                                                                  self.env.demands, aug_factor)
        ...
        for j, pref in enumerate(self.prefs):
            ...
            with torch.no_grad():
                ...
                self.model.pre_forward(reset_state, self.env.pomo_size, pref, encode)
            ...
            # Scale rewards
            scaling1 = 1 #self.ref[0]
            scaling2 = 1 #self.ref[1]
            reward[:, :, 0] = reward[:, :, 0] / scaling1
            reward[:, :, 1] = reward[:, :, 1] / scaling2
