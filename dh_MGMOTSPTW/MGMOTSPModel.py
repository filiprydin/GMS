...
from dh_MGMOTSPTW.Decoding.pruning_decoder import PruningDecoder
...
class TSPModel(nn.Module):
    ...
    def encode(self):
        ...
        edge_to_node = self.env.edge_to_node.to(dtype=torch.int64) # (B, E, 2)
        tw_start = self.env.tw_start # (B, N)
        tw_end = self.env.tw_end # (B, N)
        ...
        edge_attr = torch.cat((edge_attr, tw_start_features, tw_end_features), dim = 2)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder_2(state.current_node, state.ninf_mask, state.current_time)
